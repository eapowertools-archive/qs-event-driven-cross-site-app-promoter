...
if PROMOTION_EMAIL_ALERTS.lower() == "true":
    try:
        ...
        LOGGER.error("%s\tSomething went wrong while trying to setup email alerts: %s", LOG_ID, error)
else:
    EMAIL_ALERTS = False
...
def email_on_publish_to_review(app_id):
    ...
    if EMAIL_ALERTS:
        ...
        stream_full_status, stream_full_response = qrs.stream_full(s, base_url, stream_id)
        qrs.close_requests_session(s)
        ...
        LOGGER.debug("%s\tStream custom properties: %s", log_id, stream_custom_properties)
        ...
        if stream_marked_for_approval:
            ...
            HANDLER.setFormatter(formatter)
            LOGGER.addHandler(HANDLER)
            LOGGER.info("%s\tApp published to approval stream: '%s', configuring email", log_id, stream_name)

            try:
                EMAIL_MAP_FILE_DIRECTORY = str(Path(__file__).parent.parent).replace("\\", "/")
                EMAIL_MAP_FILE = EMAIL_MAP_FILE_DIRECTORY + '/ApprovalStreamsToEmailAddressMap.csv'
                ...
                try:
                    ...
                    LOGGER.error("%s\tThere was an error trying to send the email %s", log_id, error)

            except Exception as error:
                LOGGER.error("%s\tSomething went wrong with the email alerts: %s", log_id, error)
                LOGGER.error(
