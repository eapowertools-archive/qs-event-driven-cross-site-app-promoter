...
def email_approval_status(app_name, user_id, modified_by_user, approved=True):
    ...
    LOGGER.info("%s\tRequesting user info on '%s' who owns '%s'", log_id, user_id, app_name)
    ...
    if user_full_Status != 200:
        LOGGER.error("%s\tSomething went wrong while trying to get user/full: %s", log_id, user_full_Status)
    else:
    ...
    if user_email_found:
        if approved:
            subject = "'{}' promotion approved by: '{}'".format(app_name, modified_by_user)
            ...
            subject = "'{}' promotion denied by: '{}'".format(app_name, modified_by_user)
